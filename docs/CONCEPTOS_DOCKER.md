...
|---|---|---|
| `front` | **Sí**, en `./front_flask` | Es código **suyo**: nadie más lo tiene, hay que armarlo |
| `api-generica` | **Sí**, en `./api_generica` | Es código **suyo**: nadie más lo tiene, hay que armarlo |
| `api-facturas` | **Sí**, en `./api_facturas` | Es código **suyo**: nadie más lo tiene, hay que armarlo |
| `api-generica-csharp` | **Sí**, en `./api_generica_csharp` | Es código **suyo**: nadie más lo tiene, hay que armarlo |
...
flowchart LR
    ...
    FRONT -->|"http://api-facturas:8012<br/>al controlador, por el NOMBRE"| APIFACTURAS
    FRONT -->|"http://api-generica:8011<br/>al controlador, por el NOMBRE"| APIGENERICA
    FRONTBLAZOR -->|"api-generica (por el NOMBRE)"| APIGENERICA
    FRONTBLAZOR -->|"http://api-generica-csharp:8013<br/>al controlador, por el NOMBRE"| APIGENERICACSHARP
    PHPMYADMIN -->|"mariadb:3306 (DNS de Docker)"| MARIADB
    SQLSERVERINIT -->|"espera el healthcheck,<br/>siembra y termina"| SQLSERVER
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13316"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15448"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11443"| SQLSERVER
```
...
```yaml
URL_API: http://api-generica:8011
```
...
**`api-generica` es un nombre que solo existe dentro de la red de Docker.**
Su navegador corre en Windows, fuera de esa red, y no sabe quién es.

| Desde dónde | Qué dirección sirve | Por qué |
|---|---|---|
| **Su navegador** | `http://localhost:8011` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-generica:8011` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8011:8011`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8011`, el front
...
```powershell
docker network inspect proyecto_construccion1_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_construccion1` produce
la red `proyecto_construccion1_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_construccion1-front-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_construccion1_default has active endpoints (name:"proyecto_construccion1-front-1" …)
```
...
> contenedor lo alcanza: desde `front` de este proyecto,
...
```bash
# ¿«front» alcanza la base de SU propio proyecto? Le pregunta por el nombre
...
docker compose exec front getent hosts postgres
...
docker compose exec front curl -s -o /dev/null -w "%{http_code}\n" \
