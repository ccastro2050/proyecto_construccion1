...
{
    var creado = await servicio.CrearProductoAsync(peticion);
    // (sin Assert: si CrearProductoAsync guarda mal, esta prueba PASA)
}
```
...
```csharp
[Fact]
public async Task CrearProducto_GuardaElNombre()
{
    var creado = await servicio.CrearProductoAsync(peticion);
