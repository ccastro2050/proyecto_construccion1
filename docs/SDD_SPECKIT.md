...
## 3b. Los 8 documentos del spec kit (los de `api_facturas/docs/spec_kit/`)
...
### 3b.1 Los 8 documentos: qué es, para qué sirve y cómo se hace cada uno
...
Los valores viajan como :parametros de SQLAlchemy text(); JAMÁS se
concatenan. `f"WHERE codigo = '{codigo}'"` es inyección esperando turno.
```
...
| Archivo | Qué crece |
|---|---|
| `servicios/ensamblador.py` | ★ el cableado de la rebanada nueva |
| `requirements.txt` | ★ un paquete nuevo, si la versión lo exige |
```
...
```bash
curl http://localhost:8012/api/producto        # total: 8
curl -i http://localhost:8012/api/producto/PR999   # → 404
```
...
en un estado COMPROBABLE (arrancar la API y abrir `/docs`); la verificación se escribe
como comando concreto, no como "revisar que funcione".

```markdown
## Fase 2 — El modelo y las peticiones
- [ ] modelos/producto.py (la entidad: 4 campos tipados)
- [ ] peticiones/producto_crear.py (Pydantic: todo obligatorio)

**Verificar:** la API arranca y /docs muestra el esquema nuevo.
```
