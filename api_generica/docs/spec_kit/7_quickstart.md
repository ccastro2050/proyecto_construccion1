...
(En Docker: `docker build -t api-generica . ; docker run -p 8011:8011 -e DB_PROVIDER=... -e DB_POSTGRES=... api-generica`.)

## 3. Smoke test (5 minutos)

```powershell
# 1. Diagnóstico y documentación
curl http://localhost:8011/            # {"mensaje":"API CRUD genérica funcionando",...}
# abrir http://localhost:8011/swagger en el navegador

# 2. Lecturas genéricas — la MISMA ruta sirve para cualquier tabla
curl http://localhost:8011/api/producto
curl http://localhost:8011/api/persona
curl http://localhost:8011/api/factura/numero/1       # conversión texto→int automática
curl -i http://localhost:8011/api/factura/numero/99   # 404

# 3. Ciclo CRUD sobre persona
curl -X POST http://localhost:8011/api/persona -H "Content-Type: application/json" `
     -d '{\"codigo\":\"P999\",\"nombre\":\"Test\",\"email\":\"t@t.co\",\"telefono\":\"300\"}'
curl -X PUT  http://localhost:8011/api/persona/codigo/P999 -H "Content-Type: application/json" `
     -d '{\"nombre\":\"Test Editado\"}'
curl http://localhost:8011/api/persona/codigo/P999
curl -X DELETE http://localhost:8011/api/persona/codigo/P999

# 4. BCrypt de extremo a extremo
curl -X POST "http://localhost:8011/api/usuario?campos_encriptar=contrasena" `
     -H "Content-Type: application/json" -d '{\"email\":\"qa@test.com\",\"contrasena\":\"secreto1\"}'
curl -X POST "http://localhost:8011/api/usuario/verificar-contrasena?campo_usuario=email&campo_contrasena=contrasena&valor_usuario=qa@test.com&valor_contrasena=secreto1"   # 200
curl -i -X POST "http://localhost:8011/api/usuario/verificar-contrasena?campo_usuario=email&campo_contrasena=contrasena&valor_usuario=qa@test.com&valor_contrasena=mala"   # 401
curl -X DELETE http://localhost:8011/api/usuario/email/qa@test.com

# 5. Los errores de la BD llegan legibles
curl -i -X DELETE http://localhost:8011/api/persona/codigo/P001   # 500: FK (P001 es cliente)
```
