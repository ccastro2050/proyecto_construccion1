...
dotnet run          # appsettings.json ya apunta a localhost:15432 con DatabaseProvider=Postgres

# Opción Docker (el Dockerfile del proyecto; puerto 8003 con dotnet watch):
docker build -t api-generica-csharp .
docker run -d -p 8003:8003 `
  -e ConnectionStrings__Postgres="Host=host.docker.internal;Port=15432;Database=bdfacturas_postgres_local;Username=paradigmas;Password=paradigmas123;" `
  api-generica-csharp
```
...
curl http://localhost:8003/                    # bienvenida
# abrir http://localhost:8003/swagger (botón Authorize) y /redoc

# 2. El endpoint protegido SIN token → 401 (es el contrato, no un error)
curl -i http://localhost:8003/api/producto
...
$t = Invoke-RestMethod -Uri http://localhost:8003/api/Autenticacion/token -Method Post -ContentType "application/json" -Body $body
$h = @{ Authorization = "Bearer $($t.token)" }

# 4. Con token → 200 con envoltura {tabla, esquema, limite, total, datos}
Invoke-RestMethod -Uri http://localhost:8003/api/producto -Headers $h

# 5. Lecturas por clave (anónimas) — conversión texto→tipo automática
curl http://localhost:8003/api/factura/numero/1
curl -i http://localhost:8003/api/factura/numero/99          # 404

# 6. Ciclo CRUD sobre persona (anónimo)
curl -X POST http://localhost:8003/api/persona -H "Content-Type: application/json" `
     -d '{\"codigo\":\"P999\",\"nombre\":\"Test\",\"email\":\"t@t.co\",\"telefono\":\"300\"}'
curl -X PUT  http://localhost:8003/api/persona/codigo/P999 -H "Content-Type: application/json" `
     -d '{\"nombre\":\"Test Editado\"}'
curl http://localhost:8003/api/persona/codigo/P999
curl -X DELETE http://localhost:8003/api/persona/codigo/P999

# 7. BCrypt de extremo a extremo
curl -X POST "http://localhost:8003/api/usuario?camposEncriptar=contrasena" `
     -H "Content-Type: application/json" -d '{\"email\":\"qa@test.com\",\"contrasena\":\"secreto1\"}'
curl -X POST "http://localhost:8003/api/usuario/verificar-contrasena?campoUsuario=email&campoContrasena=contrasena&valorUsuario=qa@test.com&valorContrasena=secreto1"   # 200
curl -X DELETE http://localhost:8003/api/usuario/email/qa@test.com

# 8. Consulta SELECT parametrizada
curl -X POST http://localhost:8003/api/consultas/ejecutarconsultaparametrizada `
     -H "Content-Type: application/json" `
     -d '{\"consulta\":\"SELECT * FROM producto WHERE stock > @minimo\",\"parametros\":{\"minimo\":20}}'

# 9. Metadatos y diagnóstico
curl http://localhost:8003/api/estructuras/producto/modelo
curl http://localhost:8003/api/diagnostico/conexion          # ¿a qué BD estoy conectado realmente?
```
...
| 500 con detalle de conexión | BD abajo o cadena apuntando mal (dentro de Docker: hosts `postgres`/`mariadb`/`sqlserver`; fuera: `localhost:15432/13306/11433`) |
