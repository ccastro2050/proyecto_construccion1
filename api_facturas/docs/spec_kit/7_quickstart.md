...
(En Docker: `docker build -t api-facturas . ; docker run -p 8002:8002 -e DB_PROVIDER=... -e DB_POSTGRES=... api-facturas` —
...
# 2. Lecturas
curl http://localhost:8002/api/persona/          # 6 personas
curl http://localhost:8002/api/persona/P001      # Ana Torres
curl -i http://localhost:8002/api/persona/ZZZ    # 404 estructurado

# 3. Validación Pydantic (la razón de ser de esta API)
curl -i -X POST http://localhost:8002/api/persona/ -H "Content-Type: application/json" -d '{\"codigo\":\"P999\"}'
# → 422 (falta "nombre")
...
# 5. BCrypt
curl -X POST http://localhost:8002/api/usuario/ -H "Content-Type: application/json" `
     -d '{\"email\":\"qa@test.com\",\"contrasena\":\"secreto1\"}'
curl -X POST "http://localhost:8002/api/usuario/verificar-contrasena?valor_usuario=qa@test.com&valor_contrasena=secreto1"   # 200
curl -i -X POST "http://localhost:8002/api/usuario/verificar-contrasena?valor_usuario=qa@test.com&valor_contrasena=mala"   # 401
curl -X DELETE http://localhost:8002/api/usuario/qa@test.com

# 6. El trigger de la BD trabaja
curl http://localhost:8002/api/producto/PR003                     # anotar stock
curl -X POST http://localhost:8002/api/productosporfactura/ -H "Content-Type: application/json" `
     -d '{\"fknumfactura\":1,\"fkcodproducto\":\"PR003\",\"cantidad\":1,\"subtotal\":0}'
curl http://localhost:8002/api/producto/PR003                     # stock bajó 1
curl http://localhost:8002/api/factura/1                          # total subió
curl -X DELETE http://localhost:8002/api/productosporfactura/1/PR003
```

O todo lo anterior con clics en **http://localhost:8002/docs**.
